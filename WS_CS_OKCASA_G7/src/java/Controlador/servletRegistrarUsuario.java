...
public class servletRegistrarUsuario extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Usuario usuario = new Usuario(0,rut,nombre,nombre_usuario,contrasenia,0);
            UsuarioDAO dao = new UsuarioDAO();
            if(dao.agregarUsuario(usuario)){
                request.setAttribute("msj", "Usuario Registrado");
                request.getRequestDispatcher("registrarse.jsp").forward(request, response);
            }
            else {
                request.setAttribute("err", "Usuario NO Registrado");
                request.getRequestDispatcher("registrarse.jsp").forward(request, response); 
            }
        } catch (Exception e) {
            request.setAttribute("err", "Usuario NO Registrado "+e.getMessage());
            request.getRequestDispatcher("registrarse.jsp").forward(request, response);
        }
    }
